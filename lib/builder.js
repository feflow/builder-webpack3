...
class Builder {
    ...
    static createDevConfig(options) {
        ...
        devPlugins.push(Builder._setExternalPlugin(options.externals, 'development'));
        // 设置NODE_ENV 为 development
        devPlugins.push(Builder._setDefinePlugin('development'));
        // 多实例构建
        devPlugins.push(Builder._setHappyPack());
        // 多页面打包
        ...
        devConfig.resolveLoader = Builder._setResolveLoaderPath();
    ...
    static createProdConfig(options) {
        ...
        // React, react-dom 通过cdn引入
        prodPlugins.push(Builder._setExternalPlugin(options.externals, 'production'));
        // Inline 生成出来的css
        ...
        // 多实例构建
        prodPlugins.push(Builder._setHappyPack());
        ...
        // 设置 loader 的npm包查找的相对路径，此处设置在全局的 .feflow 目录下
        prodConfig.resolveLoader = Builder._setResolveLoaderPath();

        return prodConfig;
    }

    static _setHappyPack() {
     ...
     * 开发环境使用非压缩版本的公共库，优先使用 feflow.json里面传入的externals
     * 
     * @private
     */
    static _setExternalPlugin(externals, env) {
        const defaultExternals = env === 'development' ? [
            {
                module: 'react',
                entry: '//11.url.cn/now/lib/16.2.0/react.js?_bid=3123',
                global: 'React'
            }, {
                module: 'react-dom',
                entry: '//11.url.cn/now/lib/16.2.0/react-dom.js?_bid=3123',
                global: 'ReactDOM'
            }
        ] : [
            {
        ...
        const newExternals = externals || defaultExternals;
