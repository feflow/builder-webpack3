...
class Builder {
    ...
    static createProdConfig(options) {
        ...
        let prodPlugins = [];
        // 清空Public目录插件
        prodPlugins.push(new CleanWebpackPlugin([path.join(projectRoot, 'public/')]));
        // 设置提取CSS为一个单独的文件的插件
        ...
        // 由于静态文件打包到了 cdn 目录，assets路径引用问题采用全局替换策略进行修复
        prodPlugins.push(Builder._setReplacePlugin(new RegExp(assetsPrefix, 'g'), ''));
     ...
     * @returns {ReplaceBundlePlugin}
     * @private
     */
    static _setReplacePlugin(regex, replaceWith) {
