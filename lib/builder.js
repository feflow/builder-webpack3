...
class Builder {
     ...
     * @param {String}  options.bizName                 业务名称 (如果不传，则放在跟目录)
     * @param {String}  options.inject                  是否注入chunks
     * @example
     */
    static createProdConfig(options) {
        ...
        const assetsPrefix = bizName ? `cdn/${bizName}` : 'cdn';
        const cdnUrl = bizName ? `//${cdn}/${product}/${moduleName}/${bizName}` : `//${cdn}/${product}/${moduleName}`;
        const serverUrl = bizName ? `//${domain}/${moduleName}/${bizName}` : `//${domain}/${moduleName}`;
                ...
                rules: [
                    {
                        ...
                        // 并且替换为
                        // <script>${require('raw-loader!../../node_modules/@tencent/report
                        // - whitelist')}</script> 语法
                        ...
                        replacement: (source) => {
                            ...
                            return "<script>${require('raw-loader!" + pkg + "')}</script>";
                        }
                    }, {
     ...
     */
    static _setJsRule() {
        return {test: /\.js$/, exclude: /(node_modules|bower_components)/, loader: 'happypack/loader'};
    }
