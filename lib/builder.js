...
class Builder extends BuilderCore {
    ...
    static createDevConfig(options) {
        ...
        const publicPath = `//now.qq.com/${moduleName}/${bizName}/`;
        ...
        // 多实例构建
        devPlugins.push(this.setHappyPack(options.jsLoader));
        // 增加热更新组件
        ...
        // 这里还是依然使用生产环境所使用的publicPath
        // 这样可以使得开发环境中只需要配一个代理，即xxx.com/modulename/bizname/pagename/** -> 127.0.0.1
        // 之后他们所请求的相对路径静态资源都可以找到，比如xxx.com/modulename/bizname/pagename/index.js|css|img
        devConfig.output = this.setOutput(false, '', publicPath);
    ...
    static createProdConfig(options) {
        ...
        // 多实例构建
        prodPlugins.push(this.setHappyPack(options.jsLoader));
        // 抽离公共js
