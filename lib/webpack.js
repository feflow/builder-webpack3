 ...
 *
 * lewischeng <lewischeng@tencent.com>
 * 如果你的代码没有我写的优雅, 那么请默默在内心表达对我的崇高敬意吧~
 */
...
const baseConfig = {
    ...
    plugins: [],
    resolve: {
        alias: glob.sync('./src/**/')                  // 支持Webpack import绝度路径的写法
    }
};
...
class Webpack {
    ...
    static createDevConfig(options) {
        ...
        // 多页面打包
        const { newEntry, htmlWebpackPlugins} = Webpack._setMultiplePage(devConfig.entry, false, false, '', '');
    ...
    static createProdConfig(options) {
        ...
        // 多页面打包, 默认压缩html
        const { newEntry, htmlWebpackPlugins} = Webpack._setMultiplePage(prodConfig.entry, options.minifyHTML || true, true, false, assetsPrefix, htmlPrefix);
        ...
        Object.keys(entries).map((index) => {
            ...
            htmlWebpackPlugins.push(
                new HtmlWebpackPlugin({
                    ...
                    template: path.join(process.cwd(), `src/pages/${pageName}/index.html`),
    ...
    static _setOffline(assetsPrefix, htmlPrefix, cdnUrl, serverUrl) {
        return new ZipWebpackPlugin({
            path: path.join(process.cwd(),'./public/offline'),
    ...
    static _setAlias() {
        ...
        listDir(path.join(process.cwd(), './src'), 1)
            .forEach((dir) => {
